...
const isForcingUpdateKey = newSymbol(
  'isForcingUpdate',
);
...
function reportRendering(component) {
  ...
  renderReporter.emit({
    event: 'render',
    renderTime:
      component.__$mobRenderEnd -
      component.__$mobRenderStart,
    totalTime:
      Date.now() - component.__$mobRenderStart,
    component: component,
    node: node,
  });
}
...
export function useStaticRendering(
  useStaticRendering,
) {
...
function makeObservableProp(target, propName) {
  const valueHolderKey = newSymbol(
    `reactProp_${propName}_valueHolder`,
  );
  const atomHolderKey = newSymbol(
    `reactProp_${propName}_atomHolder`,
  );
  function getAtom() {
  ...
  Object.defineProperty(target, propName, {
    ...
    set: function set(v) {
      if (
        !this[isForcingUpdateKey] &&
        !shallowEqual(this[valueHolderKey], v)
      ) {
...
export function observer(arg1, arg2) {
  ...
  target.render = function() {
    return makeComponentReactive.call(
      this,
      baseRender,
    );
  };
...
function mixinLifecycleEvents(target) {
  ...
  if (!target.shouldComponentUpdate) {
    target.shouldComponentUpdate =
      reactiveMixin.shouldComponentUpdate;
  } else {
    if (
      target.shouldComponentUpdate !==
      reactiveMixin.shouldComponentUpdate
...
export const Observer = observer(
  ({ children, inject: observerInject, render }) => {
    ...
    const InjectComponent = inject(observerInject)(
      component,
    );
  ...
  return memo(props => {
    ...
    const reaction = useMemo(
      () =>
        ...
        // force a new render
        new Reaction(
          `observer(${baseComponent.displayName ||
            baseComponent.name})`,
          forceUpdate,
        ),
      [],
