...
class Provider extends Component {
  ...
  constructor(props, context) {
    ...
    if (!warnedAboutProvider) {
      ...
      if (supportsContext) {
        warning +=
          ' Please use React.createContext instead.';
      } else {
        warning +=
          ' Please update to React 16.3 to use React.createContext instead.';
      }
  ...
  static getDerivedStateFromProps(
    ...
    if (
      Object.keys(nextProps).filter(validStoreName)
        .length !==
      Object.keys(prevState).filter(validStoreName)
        .length
    )
      console.warn(
